...
class CN_QuantifierSegmenter implements ISegmenter {
    ...
    static {
        final char[] ca = CN_QuantifierSegmenter.Chn_Num.toCharArray();
        for (final char nChar : ca) {
            CN_QuantifierSegmenter.ChnNumberChars.add(nChar);
        }
    }
    /*
     * 词元的开始位置，
    ...
    public void analyze(final AnalyzeContext context) {
        ...
        if ((this.nStart == -1) && (this.nEnd == -1) && this.countHits.isEmpty()) {
            // 对缓冲区解锁
            context.unlockBuffer(CN_QuantifierSegmenter.SEGMENTER_NAME);
        } else {
            context.lockBuffer(CN_QuantifierSegmenter.SEGMENTER_NAME);
        }
    }
     ...
     * @param context
     */
    private void processCount(final AnalyzeContext context) {
        // 判断是否需要启动量词扫描
        ...
        if (CharacterUtil.CHAR_CHINESE == context.getCurrentCharType()) {
            ...
            if (!this.countHits.isEmpty()) {
                // 处理词段队列
                final Hit[] tmpArray = this.countHits.toArray(new Hit[this.countHits.size()]);
            ...
            // *********************************
            // 对当前指针位置的字符进行单字匹配
            final Hit singleCharHit = Dictionary.getSingleton().matchInQuantifierDict(context.getSegmentBuff(), context.getCursor(), 1);
            if (singleCharHit.isMatch()) {// 首字成量词词
                // 输出当前的词
                final Lexeme newLexeme = new Lexeme(context.getBufferOffset(), context.getCursor(), 1, Lexeme.TYPE_COUNT);
                context.addLexeme(newLexeme);
                // 同时也是词前缀
     ...
     * @return
     */
    private boolean needCountScan(final AnalyzeContext context) {
        if (((this.nStart != -1) && (this.nEnd != -1)) || !this.countHits.isEmpty()) {
            // 正在处理中文数词,或者正在处理量词
            return true;
        } else {
            ...
            if (!context.getOrgLexemes().isEmpty()) {
                final Lexeme l = context.getOrgLexemes().peekLast();
     ...
     * @param context
     */
    private void outputNumLexeme(final AnalyzeContext context) {
        if ((this.nStart > -1) && (this.nEnd > -1)) {
            // 输出数词
            final Lexeme newLexeme = new Lexeme(context.getBufferOffset(), this.nStart, this.nEnd - this.nStart + 1, Lexeme.TYPE_CNUM);
            context.addLexeme(newLexeme);
        }
    }
}
