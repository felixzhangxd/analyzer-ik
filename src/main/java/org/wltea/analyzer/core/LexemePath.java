...
class LexemePath extends QuickSortSet implements Comparable<LexemePath> {
     ...
     * @return
     */
    boolean addCrossLexeme(final Lexeme lexeme) {
        if (this.isEmpty()) {
     ...
     * @return
     */
    boolean addNotCrossLexeme(final Lexeme lexeme) {
        if (this.isEmpty()) {
            this.addLexeme(lexeme);
            this.pathBegin = lexeme.getBegin();
            this.pathEnd = lexeme.getBegin() + lexeme.getLength();
            ...
            this.addLexeme(lexeme);
            this.payloadLength += lexeme.getLength();
            final Lexeme head = this.peekFirst();
            this.pathBegin = head.getBegin();
            final Lexeme tail = this.peekLast();
     ...
     */
    Lexeme removeTail() {
        final Lexeme tail = this.pollLast();
        if (this.isEmpty()) {
            this.pathBegin = -1;
            this.pathEnd = -1;
            this.payloadLength = 0;
            ...
            final Lexeme newTail = this.peekLast();
     ...
     * @return
     */
    boolean checkCross(final Lexeme lexeme) {
    ...
    LexemePath copy() {
        final LexemePath theCopy = new LexemePath();
    ...
    @Override
    public int compareTo(final LexemePath o) {
        // 比较有效文本长度
