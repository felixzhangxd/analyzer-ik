...
class AnalyzeContext {
    ...
    public AnalyzeContext(final Configuration cfg) {
        this.cfg = cfg;
        this.segmentBuff = new char[AnalyzeContext.BUFF_SIZE];
        this.charTypes = new int[AnalyzeContext.BUFF_SIZE];
     ...
     * @throws IOException
     */
    int fillBuffer(final Reader reader) throws IOException {
        int readCount = 0;
        if (this.buffOffset == 0) {
            // 首次读取reader
            readCount = reader.read(this.segmentBuff);
        } else {
            final int offset = this.available - this.cursor;
            if (offset > 0) {
            ...
            readCount += reader.read(this.segmentBuff, offset, AnalyzeContext.BUFF_SIZE - offset);
        }
     ...
     * @param segmenterName
     */
    void lockBuffer(final String segmenterName) {
     ...
     * @param segmenterName
     */
    void unlockBuffer(final String segmenterName) {
     ...
     * @param lexeme
     */
    void addLexeme(final Lexeme lexeme) {
     ...
     * @param path
     */
    void addLexemePath(final LexemePath path) {
        if (path != null) {
    ...
    void outputToResult() {
        ...
        for (; index <= this.cursor;) {
            ...
            final LexemePath path = this.pathMap.get(index);
            if (path != null) {
     ...
     * @param index
     */
    private void outputSingleCJK(final int index) {
        if (CharacterUtil.CHAR_CHINESE == this.charTypes[index]) {
            final Lexeme singleCharLexeme = new Lexeme(this.buffOffset, index, 1, Lexeme.TYPE_CNCHAR);
            this.results.add(singleCharLexeme);
        } else if (CharacterUtil.CHAR_OTHER_CJK == this.charTypes[index]) {
            final Lexeme singleCharLexeme = new Lexeme(this.buffOffset, index, 1, Lexeme.TYPE_OTHER_CJK);
            this.results.add(singleCharLexeme);
        }
    }
     ...
     * 组合词元
     */
    private void compound(final Lexeme result) {
        ...
        if (!this.results.isEmpty()) {
            if (Lexeme.TYPE_ARABIC == result.getLexemeType()) {
                final Lexeme nextLexeme = this.results.peekFirst();
