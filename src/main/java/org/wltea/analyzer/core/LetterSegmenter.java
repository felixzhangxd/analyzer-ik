...
class LetterSegmenter implements ISegmenter {
    ...
    LetterSegmenter() {
        Arrays.sort(LetterSegmenter.Letter_Connector);
        Arrays.sort(LetterSegmenter.Num_Connector);
        this.start = -1;
        this.end = -1;
        this.englishStart = -1;
        this.englishEnd = -1;
    ...
    public void analyze(final AnalyzeContext context) {
        ...
        // 判断是否锁定缓冲区
        if (bufferLockFlag) {
            context.lockBuffer(LetterSegmenter.SEGMENTER_NAME);
        } else {
            // 对缓冲区解锁
            context.unlockBuffer(LetterSegmenter.SEGMENTER_NAME);
        }
    }
     ...
     * @return
     */
    private boolean processMixLetter(final AnalyzeContext context) {
        ...
        if (this.start == -1) {// 当前的分词器尚未开始处理字符
            ...
            if ((CharacterUtil.CHAR_ARABIC == context.getCurrentCharType()) || (CharacterUtil.CHAR_ENGLISH == context.getCurrentCharType())) {
                ...
                // 遇到非Letter字符，输出词元
                final Lexeme newLexeme = new Lexeme(context.getBufferOffset(), this.start, this.end - this.start + 1, Lexeme.TYPE_LETTER);
     ...
     * @return
     */
    private boolean processEnglishLetter(final AnalyzeContext context) {
        boolean needLock = false;
     ...
     * @return
     */
    private boolean processArabicLetter(final AnalyzeContext context) {
        boolean needLock = false;
     ...
     * @return
     */
    private boolean isLetterConnector(final char input) {
        final int index = Arrays.binarySearch(LetterSegmenter.Letter_Connector, input);
        return index >= 0;
    }
     ...
     * @return
     */
    private boolean isNumConnector(final char input) {
        final int index = Arrays.binarySearch(LetterSegmenter.Num_Connector, input);
        return index >= 0;
    }
}
