...
public class SWMCQueryBuilder {
     ...
     * @return Lucene Query
     */
    public static Query create(final String fieldName, final String keywords, final boolean quickMode) {
        if ((fieldName == null) || (keywords == null)) {
            throw new IllegalArgumentException("参数 fieldName 、 keywords 不能为null.");
        }
        // 1.对keywords进行分词处理
        final List<Lexeme> lexemes = SWMCQueryBuilder.doAnalyze(keywords);
        // 2.根据分词结果，生成SWMCQuery
        final Query _SWMCQuery = SWMCQueryBuilder.getSWMCQuery(fieldName, lexemes, quickMode);
        return _SWMCQuery;
    }
     ...
     * @return
     */
    private static List<Lexeme> doAnalyze(final String keywords) {
        final List<Lexeme> lexemes = new ArrayList<Lexeme>();
        final IKSegmenter ikSeg = new IKSegmenter(new StringReader(keywords), true);
        try {
            Lexeme l = null;
     ...
     * @return
     */
    private static Query getSWMCQuery(final String fieldName, final List<Lexeme> lexemes, final boolean quickMode) {
        // 构造SWMC的查询表达式
        final StringBuffer keywordBuffer = new StringBuffer();
        // 精简的SWMC的查询表达式
        final StringBuffer keywordBuffer_Short = new StringBuffer();
        // 记录最后词元长度
        ...
        if (quickMode && ((shortCount * 1.0f / totalCount) > 0.5f)) {
            try {
                ...
                final Query q = qp.parse(keywordBuffer_Short.toString());
                return q;
            ...
            if (keywordBuffer.length() > 0) {
                try {
                    // System.out.println(keywordBuffer.toString());
                    final Query q = qp.parse(keywordBuffer.toString());
                    return q;
                } catch (final ParseException e) {
                    e.printStackTrace();
                }
